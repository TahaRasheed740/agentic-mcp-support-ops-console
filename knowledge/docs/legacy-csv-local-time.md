---
slug: legacy-csv-local-time
title: Legacy local-time CSV exports
product_area: exports
version: "0.9"
status: superseded
published_at: 2026-05-15
superseded_by: export-timezones
---

# Legacy local-time CSV exports
...
Early CSV exports rendered timestamps using workspace local time. This article applies to Acme Automations organizations and should be read
...
Examples omit an explicit UTC offset. Record the organization ID, integration ID, relevant UTC timestamps, and
...
Ambiguous daylight saving timestamps led to the UTC export standard. Similar symptoms can have unrelated causes, so a matching title or error word
...
Follow the current Time zones in CSV exports page. Make one controlled change at a time and preserve the before-and-after run
IDs. Actions that retry work, change routing, or alter customer configuration require an explicit
support-engineer decision.

## Verification

Current exports use an explicit Z suffix. If verification fails, revert the controlled change when possible,
