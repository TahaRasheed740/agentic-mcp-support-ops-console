---
slug: legacy-signature-json
title: Legacy parsed-JSON signature example
product_area: authentication
version: "1.0"
status: superseded
published_at: 2026-05-15
superseded_by: webhook-signatures
---

# Legacy parsed-JSON signature example
...
An old example incorrectly serialized parsed JSON before signature verification. This article applies to Acme Automations organizations and should be read
...
The sample calls JSON stringify before calculating HMAC. Record the organization ID, integration ID, relevant UTC timestamps, and
...
Re-serialization changes bytes and creates intermittent verification failures. Similar symptoms can have unrelated causes, so a matching title or error word
...
Use the current raw-body signature guide. Make one controlled change at a time and preserve the before-and-after run
IDs. Actions that retry work, change routing, or alter customer configuration require an explicit
support-engineer decision.

## Verification

Current examples hash request bytes before parsing. If verification fails, revert the controlled change when possible,
