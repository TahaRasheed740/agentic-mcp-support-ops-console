...
Live investigations remain available from case detail pages when `ANTHROPIC_API_KEY` is configured. Live mode streams events from the FastAPI SSE endpoint and can execute support MCP write tools only after explicit human approval.
